...
| 2026-08-26 | **OM-0 (registrado pelo DPO/consultor):** verificação de `uploaded_by` no nISO revelou **9 evidências centrais** (Escopo SGSI, POL-ISP-001, RACI, AWS IAM/MFA/VPC/rede/backup) carregadas sob **`consultant@ness.io`** — a **ness atuou como implementadora**. Isso **enfraquece materialmente** a defesa de segregação de equipe (autorrevisão organizacional). Divulgação **obrigatória** ao organismo certificador junto com `GOV-AUD-001`. **Recomendação da consultoria: designar auditor 100% independente (fora de Bekaa E ness).** Decisão da direção pendente. |
