...
| Gravar **`status`** de fase da trilha | ✅ SIM | `PUT /api/v1/projects/{id}/phases/{phase_number}` · `{"status":"..."}` → `{"ok":true}` |
| Gravar **`notes`** de fase da trilha | ✅ **SIM** (liberado pelo nISO em 2026-08-27) | `PUT /api/v1/projects/{id}/phases/{phase_number}` · `{"notes":"..."}` → `{"ok":true}`. ⚠ **Endereça por `phase_number` (0..N), NÃO por `phase_id`** (id → erro "Fase não encontrada") |
...
| 2026-08-26 | **OM-0 levantado** (verificação de `uploaded_by`): 9 evidências centrais sob `consultant@ness.io` → inferido que a ness teria implementado (autorrevisão). |
| 2026-08-27 | **Relatório do auditor reclassificado:** como **não há contratação formal** do auditor, o `AUD-INT-TWYN-2026-01` é tratado como **PRÉ-AVALIAÇÃO DE PRONTIDÃO (dry-run)** — instrumento de trabalho para remediação, **não** a auditoria interna 9.2 oficial. A **9.2 formal** (evidência de certificação) permanece pendente: auditor **independente e formalmente engajado**, sobre o ISMS já remediado. Esta pré-avaliação **não** pode ser reapresentada ao organismo certificador como a 9.2. |
| 2026-08-27 | **OBS-03 (casing) — parte do repo remediada:** cabeçalhos das políticas padronizados — campo `Status` com crase em todos (35 `Aprovado`; 18 `Rascunho — pendente de aprovação`, unificando a variante "aguarda"→"pendente de" do A.8.24). O status `Approved` já fora normalizado (SoA regenerada do nISO); menções remanescentes são registros históricos. **Fica para UI/nISO:** casing de `evaluation_status` (`Conforme`→`conforme`, 3 itens) — a API do consultor não grava esse campo. |
| 2026-08-27 | **NC-04 remediada (contradição de região):** 4 documentos de trabalho ainda diziam `us-east-1` (`handoff-OPS-export-AWS`, `prompt-nISO-normas`, `analise-integrada-gaps`, `triagem-27701`). Corrigidos para **`sa-east-1`/Brasil** com nota de rastreabilidade (Correção NC-04). ISMS agora coerente: toda residência de dado = `sa-east-1`; menções remanescentes a `us-east-1` são notas de correção ou cobertura multi-região da AWS (cert.), não residência do dado TWYN. |
| 2026-08-27 | **Higiene de procedência (solicitada ao nISO):** o DPO/consultor (resper@bekaa.eu) **confirmou** que `consultant@ness.io` em 9 evidências é **dado de seed genérico** (não é domínio da ness; uploader real = a consultoria). Pedido de correção `uploaded_by` → `resper@bekaa.eu` enviado ao **admin/plataforma do nISO** (a API do consultor não grava `uploaded_by` — `PUT /evidence/{id}` só aceita `control_id`). Correção deve ficar **logada** (valor anterior/novo/motivo) por ser campo de procedência. IDs: `ev-twyn-scope/raci/mfa/priv/env/net/vpn/backup/policy`. |
| 2026-08-26 | **OM-0 RETRATADO:** o DPO/consultor confirmou que **`ness.io` NÃO é domínio da ness** — a inferência (nome ≠ entidade) estava **errada**. Sem o vínculo, não há evidência de participação da auditora na implementação; a conclusão de autorrevisão **cai**. Independência volta à posição da seção 2 (segregação + divulgação do grupo). Lição: verificar identidade de uploader antes de transformar em achado. Higiene remanescente: normalizar quem é `consultant@ness.io` no store (dado genérico/seed, a par de `u1`/`system`). |
...
| 2026-08-27 | **Evidência de ambiente AWS (dev) recebida** — documento **RESTRITO** (conta/região `sa-east-1`, coleta 27/08/2026), **NÃO versionado** neste repo público. Avaliação controle-a-controle A.8: A.8.15 (logging) e A.8.8 (varredura) **operantes**; A.8.23 **N/A** justificado; A.8.16 **parcial** (logs sem alarmes/filtros); A.8.11 **inconclusivo**; A.8.12 **não operante** + exposições de rede/acesso; A.8.29 **sem SAST/DAST**. Exposições ao vivo de severidade alta sinalizadas (acesso admin remoto aberto, MFA raiz off, política de storage permissiva, descoberta de dados sensíveis off, sem bloqueio público na conta) → tratamento imediato pela TI. |
| 2026-08-27 | **4 escritas nISO APLICADAS (aprovadas)** a partir da evidência de ambiente: A.8.15 `Missing→In Progress`; A.8.16 `Missing→In Progress`; A.8.23 `Missing→Not Applicable` (+justificativa); **A.8.29 `Implemented→In Progress` (REBAIXAMENTO** — declaração não se sustenta: ambiente sem SAST/DAST operante). Verbo `PUT /controls/{id}` `{status[,description]}`, todos `{"ok":true}`, verificados por GET. Placar 27001 = **22 Impl / 52 In Progress / 1 Planned / 2 Missing (A.8.11, A.8.12) / 16 N/A**; 27701 = 25 Impl / 6 N/A. |
| 2026-08-27 | **Gaps regerados:** `ENG-2026-001-analise-integrada-gaps-plano-acao.md` reescrito para a realidade 2026-08-27 (substitui a versão de 08-19 "0 Impl/69 Missing"). Novo veredito: adequação avançada (Marco A/integridade concluído), mas **núcleo técnico crítico em aberto** (A.8.11/8.12/8.16/8.29) + **exposições ao vivo** (P0 imediato). Detalhes sensíveis do ambiente **mantidos fora do repo público** (só nível de controle). Placar e cruzamento A.8×ambiente incorporados. |
| 2026-08-27 | **Trilha (41 fases) reconstruída + notas staged:** narrativa recuperada (6 blocos) **corrigida à fonte-verdade** (IPC 66%→**≈46%**; SoA 27001 52/27/14→**22 impl/2 Missing/16 N/A**; 27701 "21/10/0 projetado"→**25/0/6 real**; "5 Missing"→**2** (A.8.11/8.12); "27 Missing"→**2**). Afirmações sem lastro marcadas `[A CONFIRMAR]` (EVICERT/100%, TLS1.3, AES-256, 3 riscos reabertos, data 27/07). Doc `audit/ENG-2026-001-trilha-prontidao-respostas.md` com **anexo de 41 notas por fase**. **Bloqueio de sistema:** `phases.notes` **NÃO gravável pela API do consultor** (`PUT /projects/{id}/phases/{phase_id}` aceita `status`, **ignora `notes`** — `{"ok":true}` sem persistir; PATCH/subroute 404). Precisa de **liberação de sistema** (como `owner` em 08-26) p/ aplicar o lote. Probes deixaram fase 0 restaurada (notes vazio). |
| 2026-08-27 | **Pedido de sistema ao nISO redigido** (`audit/prompt-nISO-liberar-phases-notes.md`) — para o cliente encaminhar ao desenvolvedor: (1) liberar gravação de `phases.notes` pela API `consultant` (hoje no-op silencioso — reconhece a chave mas não persiste; mesmo padrão do `owner` antes de 26/08); (2) corrigir `scope` do projeto `us-east-1`→`sa-east-1` (resíduo NC-04). Inclui mapa `phase_number → phase_id` (41 fases) + preferência (Opção A: liberar e a consultoria aplica; Opção B: nISO aplica do anexo). Sem segredos. |
| 2026-08-27 | **Trilha repopulada no nISO (APLICADO, aprovado — nISO liberou `phases.notes`):** as **41 notas** da trilha gravadas via `PUT /projects/{id}/phases/{phase_number}` `{"notes":...}` — 41/41 `{"ok":true}`, **verificadas por GET (41/41 persistidas, batem com o anexo)**. Descoberta: o endpoint agora endereça por **`phase_number` (0..N)**, não por `phase_id` (id → "Fase não encontrada"). Conteúdo = anexo de `ENG-2026-001-trilha-prontidao-respostas.md` (números da fonte-verdade; `[A CONFIRMAR]` preservados até validação humana). **Ainda pendente (item 2 do pedido):** `scope` do projeto segue `us-east-1` — correção `sa-east-1` não aplicada pelo dev. |
