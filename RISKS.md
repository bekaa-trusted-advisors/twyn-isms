...
| **risk-twyn-04** | AWS IAM Credentials | Vazamento de Chaves Privadas | Uso de .env files legados | `Low` | `Mitigated` | Mitigado via conformidade técnica do controle A.5.17 Informações de Autenticação. | Security Team |
...
| **risk-twyn-03** | Bucket R2 | Exposição acidental de logs | Políticas de bucket permissivas | `Low` | `Mitigated` | Mitigado via conformidade técnica do controle A.8.12 Prevenção de Vazamento de Dados (DLP). | Security Team |
...
| **risk-twyn-05** | AWS Config | Não Conformidade com AWS FTR | AWS Config não plenamente operacional | `Low` | `Mitigated` | Mitigado via conformidade técnica do controle A.8.9 Gestão de Configuração. | Security Team |
